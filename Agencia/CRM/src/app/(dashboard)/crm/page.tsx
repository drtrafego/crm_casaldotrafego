import { getLeads, getColumns } from "@/server/actions/leads";
import { getSettings } from "@/server/actions/settings";
import { CrmView } from "@/components/features/crm/crm-view";
import { stackServerApp } from "@/stack";

export const dynamic = 'force-dynamic';

export default async function CRMPage() {
  const [leads, columns, settings, fullUser] = await Promise.all([
    getLeads(),
    getColumns(),
    getSettings(),
    stackServerApp.getUser().catch(() => null),
  ]);

  // Sanitize user object to pass only serializable data to client
  const user = fullUser ? {
    displayName: fullUser.displayName,
    primaryEmail: fullUser.primaryEmail,
    profileImageUrl: fullUser.profileImageUrl,
  } : null;

  return (
    <div className="h-[calc(100vh-100px)] flex flex-col">
      <CrmView
        initialLeads={leads as any[]}
        columns={columns}
        companyName={settings?.companyName}
        initialViewMode={settings?.viewMode || 'kanban'}
        user={user}
      />
      <div className="text-xs text-gray-300 text-center mt-2">
        CRM Global View - v2.2 (Sanitized User)
      </div>
    </div>
  );
}
